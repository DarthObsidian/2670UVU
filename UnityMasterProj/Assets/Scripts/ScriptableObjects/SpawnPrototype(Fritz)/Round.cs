...
public class Round : ScriptableObject 
{
	...
	public void EnemySpawn(GameObject _enemy, Vector3 SpawnPoint)
	{
		...
		{															//if there are multiple enimeies to spawn at once, offset them by a random amount so they dont spawn on top of eachother
	...
	{																						//
		...
		for (int i = 1; i < objToCompare.Length; i++) 
		{																					//for loop that checks each transform in the array
			if (Vector3.Distance (obj.position, objToCompare[i].position) < distClose) 
			{																				//if the new trans in closer to the obj
