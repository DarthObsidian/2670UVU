...
public class EnemySpawner : MonoBehaviour 
{
	...
	{																							//SetsUp Spawning System
		/*smallName = smallEnemySO.EnemyType;
		medName = medEnemySO.EnemyType;
		bigName = bigEnemySO.EnemyType;*/
		for (int i = 0; i < defaultEnemyPoolSize;i++) 
		{																						//creates a pool for each of the enemy types;
	...
	private void ExpandPool(List<GameObject> _pool,Enemy enemyArch, bool toActivate, Vector3 loc)
	{																									//used to create more enemies if they are needed; pool is the list to be added to, enemy arch is the type of enemy to add, to activate is wether or not it should be active (pooled or spawned), loc is where it should be spawned  
		_pool.Add (enemyArch.CreateEnemyInstance(loc, toActivate));										//Adds an enemy to the list
	}

	private GameObject FindInactive(List<GameObject> pool,Enemy _enemyArch, int lastActive)
	{																								//Finds the next inactive (not currently chasing the player) enemy by iterating through the list of enemies that its given. if none are found then it adds another by calling expandPool()
		...
		for (int i = lastActive; i < pool.Count; i++)
		{																							//starts iterating through the pooled enemies to find an inactive enemy
			if(!pool[i].activeSelf)
			{ 
				foundGO = true; lastActive = i; return pool [i];
			}																						//if an inactive enemy is found then it changes foundGO to true, returns the gameobject and breaks out of the loop (stops looking)
		}
		if (!foundGO) 
		{																							//if an the end of the list was reached before an inactive object was found then it tries again from index 0
			for (int i = 0; i < lastActive; i++) 
			{																						//looks from the start of the list to the first one checked
				if (!pool [i].activeSelf) 
				{ 
					foundGO = true;	lastActive = i; return pool [i];
		...
		if (!foundGO)
		{																							//if the entire list has been checked and nothing has been found
	...
	{																								//uses a switch statmenet to determine which kind of enemy to spawn
	...
	{																	
		...
		for(int i = roundEnemy; i < currentRound.enemies.Length; i += currentRound.enemiesPerSpawn )
		{																								//spawns the enemies in order
			Vector3 SP = currentRound.FindClosest (playerPos, enemySpawnPoints).position;               //finds the closest spawnpoint to the player
            for (int x = 0; x < currentRound.enemiesPerSpawn; x++)
			{																					//spawns enemies in bulk if needed
